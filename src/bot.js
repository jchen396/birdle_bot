...
client.on("messageCreate", async (message) => {
    ...
    var userData = JSON.parse(
        fs.readFileSync("./src/storage/userData.json", "utf8")
    );
    ...
    if (message.content.startsWith(PREFIX)) {
        ...
        if (CMD_NAME === "play") {
            ...
            for (let i = 0; i < randomNumberList.length; i++) {
                ...
                do {
                    ranNum = Math.floor(
                        Math.random() * visibleLettersList.length
                    );
            ...
            for (let i = 0; i < randomNumberList.length; i++) {
                visibleLettersList[
                    randomNumberList[i]
                ] = `:regional_indicator_${answer[randomNumberList[i]]}:`;
            }
            ...
            collector.on("collect", (msg) => {
                ...
                console.log(
                    `${
                        msg.author.username
                    } got the answer: ${answer.toUpperCase()}`
                );
                // To input point values to the winners of the word game
                fs.writeFile(
                    "./src/storage/userData.json",
            ...
            const hintButton = new MessageActionRow().addComponents(
                new MessageButton()
                    .setCustomId("hint")
                    .setLabel("Hint")
                    .setStyle("PRIMARY")
            ...
            const wordEmbed = new MessageEmbed()
                ...
                .addFields({ name: "Letters", value: `${answer.length}` });
            ...
            collector1.on("collect", async (interaction) => {
                ...
                if (userData[interaction.user.id].points >= 10) {
                    try {
                        for (let i = 0; i < answer.length; i++) {
                            if (!randomNumberList.includes(i)) {
                                console.log(
                                    ...
                                    } used a hint to find Letter ${
                                        i + 1
                                    } is ${answer[i].toUpperCase()}`
                                );
                                await interaction.deferUpdate();
                                await interaction.editReply({
                                    content: `${interaction.user.username} used a hint!`,
                                    components: [],
                                });
                        ...
                        console.error(error);
                        await message.reply(
                            "Something went wrong. Please try to not use the hint button when multiple prompts are present."
                        );
                    }
                } else {
                    const hintFailedEmbed = new MessageEmbed()
                        ...
                        .setTitle(`Hint`)
                        .setDescription(
                            `Insufficient amount of points to show hints`
                        );
                    message.reply({
                        embeds: [hintFailedEmbed],
                        emphemeral: true,
                    });
                }
            });
        ...
        if (CMD_NAME === "points") {
            if (args[0] !== undefined) {
                ...
                message.reply(
                    `${user.username} currently has **${
                        userData[args[0].slice(3, -1)].points
                    }** points`
                );
            } else {
                message.reply(
        ...
        if (CMD_NAME === "pet") {
            if (userData[message.author.id].points >= 1000) {
                const exampleEmbed = new MessageEmbed()
                    ...
                    .setFields({ name: "Points lost", value: "1000" })
                    .setThumbnail(
                        "https://cdn.drawception.com/images/panels/2017/10-19/T5gxcwASOP-6.png"
                    );
                message.channel.send({ embeds: [exampleEmbed] });
